...
transform_player_stats <- function(pbp_data, season) {
  ...
  passing_stats <- all_dropbacks %>%
    ...
    summarise(
      ...
      pass_wpa = round(sum(ifelse(qb_scramble != 1, wpa, 0), na.rm = TRUE), 4),
      ...
      pass_yac_epa_total = round(sum(ifelse(pass_attempt == 1 & qb_scramble != 1 & sack != 1, yac_epa, 0), na.rm = TRUE), 4),
      pass_yac_wpa_total = round(sum(ifelse(pass_attempt == 1 & qb_scramble != 1 & sack != 1, yac_wpa, 0), na.rm = TRUE), 4),
      .groups = "drop"
    )
  ...
  rushing_stats <- pbp_data %>%  # Use original pbp_data like reference
    ...
    summarise(
      ...
      rush_wpa_total = round(sum(wpa, na.rm = TRUE), 4),
      ...
      qb_scramble_wpa_total = round(sum(wpa[is_scramble], na.rm = TRUE), 4),
  ...
  receiving_stats <- pbp_reg %>%
    ...
    summarise(
      ...
      rec_wpa_total = round(sum(ifelse(complete_pass == 1, wpa, 0), na.rm = TRUE), 4),
      rec_success_total = sum(is_success, na.rm = TRUE),
      rec_yac_epa_total = round(sum(yac_epa, na.rm = TRUE), 4),
      rec_yac_wpa_total = round(sum(yac_wpa, na.rm = TRUE), 4),
  ...
  total_stats <- bind_rows(pass_plays, rush_plays, rec_plays) %>%
    ...
    summarise(
      ...
      total_wpa = round(sum(wpa, na.rm = TRUE), 4),
      ...
      scrim_wpa_total = round(sum(wpa[is_scrimmage], na.rm = TRUE), 4),
