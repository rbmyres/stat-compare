# Team Offense Passing Statistics Analysis 2024
# Uses nflverse play-by-play data to calculate team passing offense stats
...
# Filter for all team dropbacks (passes, sacks, spikes, scrambles)
team_dropbacks <- pbp_2024 %>%
  filter(
    season_type == "REG",
...
weekly_stats <- team_dropbacks %>%
  ...
  summarise(
    ...
    off_pass_attempts = {
      ...
      pbp_2024 %>%
        filter(season_type == "REG", week == current_week,
               (two_point_attempt != 1 | is.na(two_point_attempt)),
               posteam == team_abbr, pass_attempt == 1,
               (is.na(sack) | sack == 0)) %>%
        nrow()
    },
    ...
    off_pass_qb_hits = sum(qb_hit == 1 & sack != 1, na.rm = TRUE),
    off_pass_first_downs = sum(first_down_pass, na.rm = TRUE),
    off_pass_yac_total = sum(yac, na.rm = TRUE),
    off_pass_epa = round(sum(ifelse(qb_scramble != 1, epa, 0), na.rm = TRUE), 2),
    off_pass_wpa = round(sum(ifelse(qb_scramble != 1, wpa, 0), na.rm = TRUE), 2),
...
# Display the results
cat("Offensive Passing Statistics for:", team_abbr, "\n\n")
print(final_table)
