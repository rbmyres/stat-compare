# Team Defense Situational Statistics Analysis 2024
# Uses nflverse play-by-play data to calculate situational defensive performance
...
# Get three-and-out data (forced by defense)
three_and_outs <- pbp_2024 %>%
  ...
  group_by(week) %>%
  summarise(three_and_outs_forced = n())
...
weekly_stats <- team_defense %>%
  ...
  summarise(
    # Third down defense  
    def_third_down_attempts = {
      current_week <- first(week)
      pbp_2024 %>%
        filter(defteam == team_abbr, season_type == "REG", week == current_week, down == 3,
               play_type %in% c("pass", "run", "qb_kneel", "qb_spike"),
               (two_point_attempt != 1 | is.na(two_point_attempt))) %>%
        nrow()
    },
    ...
    # Fourth down defense
    def_fourth_down_attempts = {
      current_week <- first(week)
      pbp_2024 %>%
        filter(defteam == team_abbr, season_type == "REG", week == current_week, down == 4,
               play_type %in% c("pass", "run", "qb_kneel", "qb_spike"),
               (two_point_attempt != 1 | is.na(two_point_attempt))) %>%
        nrow()
    },
    ...
    # Early down defense (1st and 2nd down)
    def_early_down_total = sum(is_early_down, na.rm = TRUE),
    def_early_down_epa = round(sum(epa[is_early_down], na.rm = TRUE), 2),
    def_early_down_success = sum(success[is_early_down] == 1, na.rm = TRUE),
    def_early_down_wpa = round(sum(wpa[is_early_down], na.rm = TRUE), 2),
    
    # Late down defense (3rd and 4th down)
    def_late_down_total = sum(is_late_down, na.rm = TRUE),
    def_late_down_epa = round(sum(epa[is_late_down], na.rm = TRUE), 2),
    def_late_down_success = sum(success[is_late_down] == 1, na.rm = TRUE),
    def_late_down_wpa = round(sum(wpa[is_late_down], na.rm = TRUE), 2)
  ) %>%
  left_join(three_and_outs, by = "week") %>%
  mutate(
    def_three_and_outs = ifelse(is.na(three_and_outs_forced), 0, three_and_outs_forced)
  ) %>%
  select(-three_and_outs_forced) %>%
  arrange(week) %>%
...
# Display the results
cat("Defensive Situational Statistics for:", team_abbr, "\n\n")
print(final_table)
